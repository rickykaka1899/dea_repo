...
module DEA
  ...
  class Agent
    ...
    #MAX_USAGE_SAMPLES = (1*60*10)/MONITOR_INTERVAL  # 15 minutes @ 5 sec interval
    ...
    def process_droplet_status(message, reply)
      ...
      @logger.info("-----------------------process_droplet_status:#{message_json}------------------------")
      @droplets.each_value do |instances|
        instances.each_value do |instance|
    ...
    def calculate_running_info_last_1
      cpuusage = []
      memusage = []
      diskusage = []
      @logger.info("-----------------------------@usage::#{@usage}--------------------------------")
      @usage.each do |key,value|
        ...
        cpuusage<<value[len-1][:cpu]
        memusage<<value[len-1][:mem]
        diskusage<<value[len-1][:disk]
      end
      ...
      dsumusage = 0
      for i in 0...clen
        csumusage += cpuusage[i]
      end
      for i in 0...mlen
        msumusage += memusage[i]
      end
      for i in 0...dlen
    ...
    def calculate_running_info_last_10
      cpuusage = []
      memusage = []
      diskusage = []
      @logger.info("-----------------------------@usage::#{@usage}--------------------------------")
    ...
    def calculate_running_info_last_min
      cpuusage = []
      memusage = []
      diskusage = []
      @logger.info("-----------------------------@usage::#{@usage}--------------------------------")
